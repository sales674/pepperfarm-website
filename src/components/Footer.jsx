export default function Footer() {
  return (
    <footer>
      <div className="footer-top">
        <div className="footer-brand">
          <img
            src="/images/logo/Pepperfarm green logo png.png"
            alt="Pepperfarm"
            className="footer-logo"
            style={{ height: '80px', marginBottom: '24px' }}
          />
          <p>The Daily Nutrition System. Morning harvests, delivered by design for urban lives that demand more.</p>
        </div>
        <div className="footer-col">
          <h4>Menu</h4>
          <ul>
            <li><a href="#menu">Veg Bowls</a></li>
            <li><a href="#menu">Fruit Bowls</a></li>
            <li><a href="#menu">Juices</a></li>
            <li><a href="#menu">Soups</a></li>
            <li><a href="#menu">Breads</a></li>
          </ul>
        </div>
        <div className="footer-col">
          <h4>Support</h4>
          <ul>
            <li>
              <a
                href="/Pepperfarm_FAQ.pdf"
                download="Pepperfarm_FAQ.pdf"
                title="Download Pepperfarm FAQ"
              >
                FAQ ↓
              </a>
            </li>
            <li><a href="#">Delivery Zones</a></li>
            <li><a href="#">Contact</a></li>
          </ul>
        </div>
      </div>
      <div className="footer-bottom">
        <p>&copy; 2026 Pepperfarm. All rights reserved. Bengaluru, India.</p>
        <div className="footer-socials">
          <a href="#" aria-label="Instagram">IG</a>
          <a href="#" aria-label="LinkedIn">IN</a>
        </div>
      </div>
    </footer>
  );
}
